...
const handleLike = async (event, questionId) => {
  ...
  if (isclicked(btn)) {
    ...
    await updateLikes(questionId, totalLikes);
  } else {
...
const renderStats = async () => {
  let { count: questionsCount, error: questionsError } = await supabaseClient.from("Questions").select("*", { count: 'exact', head: true});
  let { count: answersCount, error: answersError } = await supabaseClient.from("Answers").select("*", {count: "exact", head: true});
  let questionsAsked = document.querySelector(".stat-info .questions-asked");
  let questionsAnswered = document.querySelector(".stat-info .questions-answered");
  questionsAnswered.textContent = answersCount || "- -";
  questionsAsked.textContent = questionsCount || "- -";
  if(questionsError || answersError) {
    displayMessage("Some error occured", true);
  }
